...
class CardbookingEntitySpec extends WordSpec with Matchers with BeforeAndAfterAll {
  ...
  private def withTestDriver(block: PersistentEntityTestDriver[CardbookingCommand[_], CardbookingEvent, CardbookingState] => Unit): Unit = {
    val driver = new PersistentEntityTestDriver(system, new CardbookingEntity, "card-booking-1")
    block(driver)
    driver.getAllIssues should have size 0
  }

  "card-booking entity" should {
